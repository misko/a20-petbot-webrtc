...
#define HTML_FILE_IDX "webrtc_oneway_idx.html"
...
static void server_callback (SoupServer *server, SoupMessage *msg, const char *path,
	...
	if (strcmp(path,"/sdp")==0) {
		//if there wasnt a cookie maybe make one?
		if (cookie == NULL) {
			gchar *id_str;
			const gchar *host;
		...
		//remove the id from the hash table because about to start streaming...
		...
		*id_ptr = id;
		...
		soup_message_set_response (msg, "application/json" , SOUP_MEMORY_STATIC, "", 0);
		soup_server_pause_message (server, msg);
	} else if (strcmp("/stream.html",path)==0) {
		fprintf(stderr,"STREAMIN?\n");
		int ret = g_file_get_contents (HTML_FILE_IDX, &contents, &length, NULL);
		if (!ret) {
			soup_message_set_status (msg, SOUP_STATUS_INTERNAL_SERVER_ERROR);
			GST_ERROR ("Error loading %s file", HTML_FILE_IDX);
			return;
		}
	...
	} else if (strncmp("/sdp_response",path,strlen("/sdp_response"))==0) {
		if (query != NULL) {
			gchar * sdp = g_hash_table_lookup(query, "sdp");
			if (sdp != NULL && mediaSession != NULL) {
				fprintf(stderr,"GOT SDP RESPONSE x2 %s\n",sdp);
